...
const WRITER_SHAPES = Object.freeze([
  ...
  },
  {
    ...
    alias_relationship: {
      ...
      accepted_aliases: ['readiness_preflight_reports'],
      alias_scope: 'path_authority_current_in_memory_readout',
      canonical_authority: 'support artifact contents contract and creation policy',
      alias_authority: 'support artifact path authority for current non-writing readiness/preflight readouts',
      export_writer_exists: false
    },
    current_fields: [
      'status',
      'generated_at',
    ...
    checks: [
      ...
      check('class_id_alias_normalization', 'readiness_preflight_export versus readiness_preflight_reports should be normalized', 'conforms', 'low',
        'readiness_preflight_export remains canonical for contents/creation policy; readiness_preflight_reports is disclosed as the accepted path-authority alias for current in-memory/readout posture.',
        'Keep the alias relationship visible if a future export writer is introduced; path authority alias must not override contents/creation authority.'),
...
function mapWriterShape(shape, contract) {
  ...
  return {
    ...
    alias_relationship: shape.alias_relationship || null,
