...
const WRITER_SHAPES = Object.freeze([
  ...
  },
  {
    ...
    checks: [
      ...
      check('free_text_length_policy', 'trace-pack free text should be bounded/truncated or summarized', 'partial', 'medium',
        'Rows are limited, but selected fields such as error_summary, error_message, warning message, task error message, and smoke artifact paths are passed through without an explicit max-length policy.',
        'Add truncation/redaction helpers for free-text diagnostics before hardening writer behavior.'),
      check('sample_limit_disclosure', 'trace pack should disclose sample limits and exclusions', 'partial', 'low',
        'The builder uses a limit and has exclusions, but the emitted pack does not include the numeric limit or per-section sample policy.',
        'Emit sample_limit and section_limit disclosures in future trace-pack manifest.'),
      check('local_path_sensitivity', 'local paths may be included only with sensitivity disclosure', 'partial', 'medium',
        'runtime.database_path, temp_root, and smoke artifact paths can be emitted; classification is high-level but local path sensitivity is not explicit.',
        'Add local_path_sensitivity disclosure and optional path redaction policy.'),
      check('provider_endpoint_secret_leakage', 'provider endpoint/error diagnostics must avoid secrets and full payloads', 'unknown', 'medium',
        'api_request_logs selects endpoint and error_message; current code does not centralize secret redaction at trace-pack assembly.',
        'Add endpoint/error redaction review before changing trace-pack writer behavior.'),
      check('queue_latest_refs_bounded_summary', 'queue latest refs should remain bounded support summary, not Discovery truth export', 'partial', 'medium',
        'latest_refs are limited and include status fields, but individual killmail refs and hashes are still emitted as samples.',
        'Add an explicit bounded-summary disclosure and consider hash/path sensitivity review.')
    ]
  },
  {
