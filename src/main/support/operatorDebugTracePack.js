...
function buildOperatorDebugTracePack(db, options = {}) {
  ...
  return {
    ...
    runtime_boundary: runtimeBoundaryStatus(db, taskRunner, limit),
...
function runtimeBoundaryStatus(db, taskRunner, limit) {
...
function fetchRunSummary(db) {
  return db.prepare(`
    ...
    FROM fetch_runs
  `).get() || { failed: 0, with_failed_expansions: 0, with_warning_summary: 0 };
}
