...
Surface discovery map: `workspace/surfaces/atlas-surface-index.md`
...
- HS97 captured storage path and budget authority decisions in `docs/current-state/current-storage-runtime-hardening.md`: real/alpha collection should lock until storage location is explicit, disk budget is physical storage use rather than scan credits, 70%/95% warn, 100% hard-locks writes/acquisition, missing storage hard-locks instead of silently relocating, and pruning is a future intelligence-formation suite.
...
- HS120 accepted `workspace/surfaces/` as advisory surface discovery input: Atlas currently consolidates toward five primary operator places, Storage widget/pop-out, External I/O widget, R-Scanner/manage pane, Live Search, and Observation pane; no UI authority or Dev runway is created.
