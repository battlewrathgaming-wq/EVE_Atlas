...
function verifyCoverageMetadata(map) {
  ...
  assert(command(map, 'watch.system_radius_setup_readout.preview').storage_action_class === 'local_db_inspection', 'system/radius setup readout should be a local DB inspection');
  assert(command(map, 'watch.system_radius_setup_readout.preview').runtime_context === 'system_radius_watch_setup_readout', 'system/radius setup readout should be classified as a readout');
  assert(command(map, 'watch.system_radius_setup_readout.preview').enforcement_status === 'read_only_non_enforcing_proof', 'system/radius setup readout should identify itself as a non-enforcing proof');
  assert(command(map, 'watch.system_radius_setup_readout.preview').external_io_dependency === 'none', 'system/radius setup readout should not declare External I/O dependency');
  assert(command(map, 'watch.system_radius_readout_readiness_bridge.preview').storage_action_class === 'local_db_inspection', 'system/radius readout/readiness bridge should be a local DB inspection');
  ...
  assert(command(map, 'watch.packet_dry_run_dispatch_parity.preview').storage_action_class === 'local_db_inspection', 'Watch packet/dry-run/dispatch parity preview should be a local DB inspection');
  ...
  assert(command(map, 'watch.bucket_product_pickup_readout.preview').storage_action_class === 'local_db_inspection', 'Watch bucket product pickup readout should be a local DB inspection');
  assert(command(map, 'watch.bucket_product_pickup_readout.preview').runtime_context === 'watch_bucket_product_pickup_readout', 'Watch bucket product pickup readout should be classified as a readout');
  assert(command(map, 'watch.bucket_product_pickup_readout.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Watch bucket product pickup readout should identify itself as a non-enforcing proof');
  assert(command(map, 'watch.bucket_product_pickup_readout.preview').external_io_dependency === 'none', 'Watch bucket product pickup readout should not declare External I/O dependency');
  ...
  assert(command(map, 'discovery.pickup_selection_contract.preview').storage_action_class === 'local_db_inspection', 'Discovery pickup selection contract should be a local DB inspection');
  assert(command(map, 'discovery.pickup_selection_contract.preview').runtime_context === 'discovery_pickup_selection_contract_readout', 'Discovery pickup selection contract should be classified as a readout');
  assert(command(map, 'discovery.pickup_selection_contract.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery pickup selection contract should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.pickup_selection_contract.preview').external_io_dependency === 'none', 'Discovery pickup selection contract should not declare External I/O dependency');
  assert(command(map, 'discovery.provider_route_packet.preview').storage_action_class === 'local_db_inspection', 'Discovery provider route packet preview should be a local DB inspection');
  assert(command(map, 'discovery.provider_route_packet.preview').runtime_context === 'discovery_provider_route_packet_preview_readout', 'Discovery provider route packet preview should be classified as a readout');
  assert(command(map, 'discovery.provider_route_packet.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery provider route packet preview should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.provider_route_packet.preview').external_io_dependency === 'none', 'Discovery provider route packet preview should not declare External I/O dependency');
  assert(command(map, 'discovery.pickup_execution_boundary.preview').storage_action_class === 'local_db_inspection', 'Discovery pickup execution boundary preview should be a local DB inspection');
  assert(command(map, 'discovery.pickup_execution_boundary.preview').runtime_context === 'discovery_pickup_execution_boundary_preview_readout', 'Discovery pickup execution boundary preview should be classified as a readout');
  assert(command(map, 'discovery.pickup_execution_boundary.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery pickup execution boundary preview should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.pickup_execution_boundary.preview').external_io_dependency === 'none', 'Discovery pickup execution boundary preview should not declare External I/O dependency');
  assert(command(map, 'discovery.dispatcher_lease_boundary.preview').storage_action_class === 'local_db_inspection', 'Discovery dispatcher lease boundary preview should be a local DB inspection');
  assert(command(map, 'discovery.dispatcher_lease_boundary.preview').runtime_context === 'discovery_dispatcher_lease_boundary_preview_readout', 'Discovery dispatcher lease boundary preview should be classified as a readout');
  assert(command(map, 'discovery.dispatcher_lease_boundary.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery dispatcher lease boundary preview should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.dispatcher_lease_boundary.preview').external_io_dependency === 'none', 'Discovery dispatcher lease boundary preview should not declare External I/O dependency');
  assert(command(map, 'discovery.candidate_ref_landing_boundary.preview').storage_action_class === 'local_db_inspection', 'Discovery candidate ref landing boundary preview should be a local DB inspection');
  assert(command(map, 'discovery.candidate_ref_landing_boundary.preview').runtime_context === 'discovery_candidate_ref_landing_boundary_preview_readout', 'Discovery candidate ref landing boundary preview should be classified as a readout');
  assert(command(map, 'discovery.candidate_ref_landing_boundary.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery candidate ref landing boundary preview should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.candidate_ref_landing_boundary.preview').external_io_dependency === 'none', 'Discovery candidate ref landing boundary preview should not declare External I/O dependency');
  assert(command(map, 'discovery.settled_receipt_boundary.preview').storage_action_class === 'local_db_inspection', 'Discovery settled receipt boundary preview should be a local DB inspection');
  assert(command(map, 'discovery.settled_receipt_boundary.preview').runtime_context === 'discovery_settled_receipt_boundary_preview_readout', 'Discovery settled receipt boundary preview should be classified as a readout');
  assert(command(map, 'discovery.settled_receipt_boundary.preview').enforcement_status === 'read_only_non_enforcing_proof', 'Discovery settled receipt boundary preview should identify itself as a non-enforcing proof');
  assert(command(map, 'discovery.settled_receipt_boundary.preview').external_io_dependency === 'none', 'Discovery settled receipt boundary preview should not declare External I/O dependency');
  ...
  assert(command(map, 'watch.operator_confirmation_contract.preview').external_io_dependency === 'none', 'Watch operator confirmation contract preview should not declare External I/O dependency');
  ...
  assert(command(map, 'watch.system_radius_authoring_preflight.preview').external_io_dependency === 'none', 'system/radius authoring preflight should not declare External I/O dependency');
  assert(command(map, 'watch.system_radius_acceptance_payload.preview').storage_action_class === 'local_db_inspection', 'system/radius acceptance payload should be a local DB inspection');
  assert(command(map, 'watch.system_radius_acceptance_payload.preview').runtime_context === 'system_radius_acceptance_payload_readout', 'system/radius acceptance payload should be classified as a readout');
  assert(command(map, 'watch.system_radius_acceptance_payload.preview').enforcement_status === 'read_only_non_enforcing_proof', 'system/radius acceptance payload should identify itself as a non-enforcing proof');
  assert(command(map, 'watch.system_radius_acceptance_payload.preview').external_io_dependency === 'none', 'system/radius acceptance payload should not declare External I/O dependency');
  assert(command(map, 'watch.create_mutation_safety_map.preview').storage_action_class === 'local_db_inspection', 'watch.create mutation safety map should be a local DB inspection');
  assert(command(map, 'watch.create_mutation_safety_map.preview').runtime_context === 'watch_create_mutation_safety_map_readout', 'watch.create mutation safety map should be classified as a readout');
  assert(command(map, 'watch.create_mutation_safety_map.preview').enforcement_status === 'read_only_non_enforcing_proof', 'watch.create mutation safety map should identify itself as a non-enforcing proof');
  assert(command(map, 'watch.create_mutation_safety_map.preview').external_io_dependency === 'none', 'watch.create mutation safety map should not declare External I/O dependency');
