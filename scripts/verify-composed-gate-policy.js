...
function verifyRepresentativeRows(preview) {
  ...
  assert(row(preview, 'runtime_enforcement_boundary_readout').effects_classification_basis.runtime_context === 'runtime_enforcement_boundary_readout', 'runtime enforcement boundary should be represented as a readout');
  assert(row(preview, 'runtime_enforcement_boundary_readout').gates.external_io.state === 'not_applicable', 'runtime enforcement boundary readout should not require External I/O');
  ...
  assert(row(preview, 'storage_authority_operator_config_write').effects_classification_basis.runtime_context === 'storage_authority_operator_config_write', 'storage authority operator config write should be represented separately');
  assert(row(preview, 'storage_authority_operator_config_write').gates.external_io.state === 'not_applicable', 'storage authority config writes should not require provider movement permission');
