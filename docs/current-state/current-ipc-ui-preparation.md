...
- Investigation lead-input ergonomics now provide empty, invalid, unresolved, and route-specific feedback; route through existing `scope.validate` for actor/system/radius scope checks; use local SDE system resolution for system names; keep numeric IDs as facts and names as labels/resolution inputs; and prefill Scopes, Actions, Queue / Enrich, and Reports without adding live/API behavior.
- Investigation stored-evidence detail now uses existing structured actor/radius report services from the first screen to show a concise read-only evidence summary and observation preview for validated leads, with honest no-evidence states and no discovery, enrichment, hydration, assessment, or watch execution.
- Investigation Queue / Enrich context now previews existing `queue.selection` for validated actor/system/radius leads, explains whether the stored discovery-scope filter has queued possible refs or needs Discover Possible Leads first, and keeps Enrich selected framed as explicit ESI expansion into stored killmail evidence. Manual expansion preflight now states provider, selected refs, caps, expected ESI calls, expected writes, evidence effect, and the separation from readability-only metadata hydration.
- Assessment Memory ergonomics now show actor-report eligibility, citation basis, cited killmail IDs, evidence window, local verification timing, and deliberate save requirements before `assessment.create`; radius reports remain context-only for this slice, and saved assessment memory stays separate from evidence, observations, discovery refs, metadata hydration, and watches.
...
- restart/reinitialization recovery now has focused offline verification covering volatile task history/locks, cancelled/running task reinitialization, failed service work diagnostics, disarmed session watch state after restart, persisted evidence/queue/fetch/API/warning reviewability, and bounded support trace output without raw evidence dumps
...
- explicit `sde.build-lookups` exclusive service command for operator-triggered local SDE lookup setup/refresh
...
- initial Investigation Desk opening view with actor/system/radius lead input, Marked/Watch distinction, passive live/API context, lead feedback, existing `scope.validate`/local-SDE resolution checks, read-only stored-evidence detail, and routes into readiness, scopes, queue/enrich, actions, reports, assessment, and task details
...
- investigation first-screen startup, lead ergonomics, stored-evidence detail, routing, and wording verification through `verify:renderer-shell` and `smoke:electron`
